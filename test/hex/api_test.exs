defmodule Hex.APITest do
  ...
  test "release" do
    ...
    {tar, _checksum} = Hex.create_tar!(meta(:pear, "0.0.1", []), [], :memory)
    ...
    {tar, _checksum} = Hex.create_tar!(meta(:grape, "0.0.2", reqs), [], :memory)
    assert {:ok, {201, _, _}} = Hex.API.Release.publish("hexpm", tar, auth)
  ...
  test "docs" do
    ...
    {tar, _checksum} = Hex.create_tar!(meta(:tangerine, "0.0.1", []), [], :memory)
    assert {:ok, {201, _, _}} = Hex.API.Release.publish("hexpm", tar, auth)
